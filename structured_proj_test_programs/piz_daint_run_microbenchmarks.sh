...
do
  ...
  do
    ...
    do
      ...
      no_proj_command="srun -n $nodes -N $nodes --tasks-per-node 1 --cpu_bind none ./zero_projections/zero_projections -i $ITERATIONS -nx $dim -bx $dim -ny $dim -by $dim -sms $sleep_us -a 225 -ll:cpu $cpus_per_node -ll:util $util_per_node -ll:io $io_per_node"
    ...
    do
      ...
      no_proj_command="srun -n $nodes -N $nodes --tasks-per-node 1 --cpu_bind none ./zero_projections/zero_projections -i $ITERATIONS -nx 1 -bx 1 -ny $dim -by $dim -sms $sleep_us -a 180 -ll:cpu $cpus_per_node -ll:util $util_per_node -ll:io $io_per_node"
