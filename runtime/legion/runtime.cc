...
namespace Legion {
  namespace Internal {
      ...
      if (!is_exclusive)
      {
                ...
                point_tasks.begin(); it != point_tasks.end(); it++)
          {
            ...
            if (functor->is_structured())
            {
              result = evaluate_structured_projection(structured_proj,
                  (*it)->get_domain_point(), req.partition, runtime->forest);
            }
            else
            {
              result = functor->project(*it, idx, req.partition,
                                        (*it)->get_domain_point());
            }
        ...
        else
        {
                ...
                point_tasks.begin(); it != point_tasks.end(); it++)
          {
            ...
            {
              result = evaluate_structured_projection(structured_proj,
                  (*it)->get_domain_point(), req.region, runtime->forest);
            }
            else
            {
              result = functor->project(*it, idx, req.region,
                                        (*it)->get_domain_point());
            }
    ...
    //--------------------------------------------------------------------------
    {
      RegionNode *ub_node = forest->get_node(upper_bound, false);
      return forest->evaluate_projection(proj, point, ub_node);
      /*
    ...
    //--------------------------------------------------------------------------
    {
      PartitionNode *ub_node = forest->get_node(upper_bound, false);
      return forest->evaluate_projection(proj, point, ub_node);
      /*
    ...
    //--------------------------------------------------------------------------
    {
      ...
      register_static_ordering_functors();
#ifdef DEBUG_LEGION
