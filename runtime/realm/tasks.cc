  ...
  void Task::execute_on_processor(Processor p)
  {
    ...
    if(ok_to_run) {
      ...
      if(measurements.wants_measurement<ProfilingMeasurements::OperationStatus>()) {
      	...
      	catch (const ExecutionException& e) { //ksmurthy forced a set of the OpStatus
          ...
          b.error_code = 42;
          measurements.add_measurement(b);
      	  thread->stop_operation(this);
          ...
          thread->stop_operation(this);
      	  thread->record_perf_counters(measurements);
          ByteArray ba("realm/tasks.cc:172", 19);
      	  mark_terminated(42, ba);
        }
        catch (...) {//ksmurthy we need a generic handler
          ProfilingMeasurements::OperationStatus b;
          b.result = ProfilingMeasurements::OperationStatus::TERMINATED_EARLY;
          b.error_code = 42;
          measurements.add_measurement(b);
          thread->stop_operation(this);
      	  thread->record_perf_counters(measurements);
          ByteArray ba("realm/tasks.cc:180", 19);
      	  mark_terminated(42, ba);
      	  //mark_finished(true);
        }
      } else
