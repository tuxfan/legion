...
#if 0 
      static void restart_execution(Processor p) {
        //Internal::ContextInterface *ctx = *((const Internal::ContextInterface) args.base() ));
        //ctx->get_task()->get_context()->increment_pending(ctx->get_task());
        return;
        Thread *thread = Thread::self();
        //ctx->increment_pending(this);
        try {
        ...
        if(ok_to_run) {
          ...
          if(measurements.wants_measurement<ProfilingMeasurements::OperationStatus>()) {
            try {
              ...
              std::cout << " ok we entered the never-goto-exception place " << std::endl;
              #if 0
              ...
              #if 1
              ApplicationException newEx(ProfilingMeasurements::OperationStatus::TERMINATED_EARLY, "we are terminating the crashed exception now", 45);
              newEx.populate_profiling_measurements(measurements);
              thread->stop_operation(this);
              mark_terminated(newEx.error_code, newEx.details);
              #else
              Realm::ByteArray *x = new ByteArray("we are terminating the crashed exception now", 45);
              thread->stop_operation(this);
              mark_terminated(ProfilingMeasurements::OperationStatus::TERMINATED_EARLY,*x); 
              //mark_terminated(ProfilingMeasurements::OperationStatus::COMPLETED_WITH_ERRORS,*x); 
              free(x);
              #endif
          ...
          #endif
          {
            try {
              ...
              //lets recurse until it finishes
              std::cout << " ok we entered the exception place " << std::endl;
              #if 0
              std::exception_ptr p = std::current_exception();
              std::cout <<(p ? p.__cxa_exception_type()->name() : "null") << std::endl;
              #endif
              ...
              //while(DEBUG_WAIT == 0); 
              if(resilient.wants_technique(RESILIENTID_RUN_UNTIL_COMPLETION)) {
                //restart_execution(p);
                mark_finished(true);
              } else {
                thread->stop_perf_counters();
