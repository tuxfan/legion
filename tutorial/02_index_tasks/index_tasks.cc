...
//  }
  ...
  volatile int recursive = 100000;
  recursive = atoi(command_args.argv[2]);
  printf("Running hello world redux for %d points and %d iterations...\n", num_points, recursive);
  while(recursive--) {
                     ...
                     Context ctx, Runtime *runtime)
{
  ...
  if(task->index_point.point_data[0] == 1) {
    //printf("Hello world from task %lld! about to throw\n", task->index_point.point_data[0]);
    //throw std::exception();
  } else {
